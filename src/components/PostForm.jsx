...
import { validationForBlogPost } from "../utilities/validation";
...
const PostForm = ({ action, post }) => {
  ...
  const { mutateAsync: createNewBlogPost, isPending: creatingNewBlogPost } =
    PostQuery.useCreateNewBlogPost();

  const { mutateAsync: deleteImage, isPending: deletingImage } =
    PostQuery.useDeleteImage();

  const { mutateAsync: updatingPost, isPending: isUpdatingPost } =
    PostQuery.useUpdatingPost();
  ...
  const handleDeleteImage = async () => {
    if (imageToDelete) {
      const publicId = imageToDelete.querySelector("img").alt;
      if (!(publicId === "imageThroughUrl")) {
        const isDeleteImage = await deleteImage(publicId);
  ...
  const handleImageUpload = async (e) => {
    ...
    reader.onload = (event) => {
      ...
      if (targetPlaceholder) {
        replacePlaceholderWithImage(
          targetPlaceholder,
          imageUploaded.imageUrl, // info of file...
          imageUploaded.publicKey
        );
      }
    };
  ...
  useEffect(() => {
    if (post && action === "edit") {
      editorRef.current.innerHTML = post?.post?.content;
    }
  }, [action, post]);
  ...
  return (
    <>
      <Box>
        <Formik
          initialValues={{
            title: post?.post?.title || "your post title",
            coverImage: post?.post?.coverImage || null,
            status: post?.post?.status || "draft",
            excerpt: post?.post?.excerpt || "",
            tags: post ? post?.post?.tags.join(",") : "",
          }}
          enableReinitialize
          onSubmit={async (values, { resetForm }) => {
            values.content = DOMPurify.sanitize(editorRef.current.innerHTML);
            if (!values.content || !values.content.trim()) {
              return;
            }

            if (post && action === "edit") {
              const isImageDeleted = await deleteImage(post.post.publicId);
              if (!isImageDeleted) {
              ...
              navigate(`/blog-post/${editPost.data.post.slug}`);
              return;
            }

            const newPost = await createNewBlogPost({
            ...
            resetForm();
            editorRef.current.innerHTML = "";
            navigate(`/blog-post/${newPost.post.slug}`);
          }}
          validationSchema={validationForBlogPost}
        >
          <Form>
            <Stack gap="3rem">
              {/* TITLE */}
                    ...
                    >
                      ...
                      <Box
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            ...
                            while (
                              codeBlockNode &&
                              codeBlockNode !== editorRef.current
                            ) {
                              if (codeBlockNode.nodeName === "PRE") {
                                inCodeBlock = true;
                                break;
                      ...
                      >
                        ...
                        <p>
                          ...
                          <strong>Chakra UI</strong> and <strong>React</strong>.
                        ...
                        <p>
                          Select text and use the toolbar above to format your
                          content.
                        </p>
                      </Box>
                    </Box>
                              ...
                              {({ meta, form }) => (
                                ...
                                >
                                  ...
                                  <RadioGroup.Root
                                    ...
                                    onValueChange={(value) => {
                                      form.setFieldValue("status", value.value);
                                    }}
                                  >
                                    <HStack gap="6">
                                      {items.map((item) => (
                                        ...
                                        >
                                          ...
                                          // Add these styles for visual feedback
                                          >
                        ...
                        <FormikField name="status">
                          {({ form }) => (
                            <Button
                              ...
                              disabled={isUpdatingPost || creatingNewBlogPost}
                              bg="green.500"
                              _hover={{ bg: "green.700" }}
                            >
                              {creatingNewBlogPost || isUpdatingPost ? (
                                <Loader />
                              ) : action === "edit" ? (
                                "update post"
                              ) : (
